...
void Obstacle::setRadius(float r) { radius = r; }

vec2 Obstacle::getPos() { return position; };

void Obstacle::setPos(vec2 pos) { position = pos; };
...
void Solver::updateColor(size_t index, Parameters params) {
  ...
  unsigned char red = static_cast<unsigned char>(normalizedVelocity * 255);
  unsigned char green = static_cast<unsigned char>(normalizedVelocity * 255);
  unsigned char blue =
      static_cast<unsigned char>((1.0f - normalizedVelocity / 1.5) * 255);
  ...
  for (auto obstacle : obstacles) {
    if (obstacle.shape == EXT_CIRCLE || obstacle.shape == INT_CIRCLE) {
      ...
      if (obstacle.shape == EXT_CIRCLE) {
        ...
        vec2 closest = {std::clamp(pos.x, rect.x, rect.x + rect.width),
